...
async function generateTestSteps(requirements) {
  const model = genAI.getGenerativeModel({
    model: cfg.model,
    systemInstruction: `Extract testable acceptance criteria from web app requirements.
Return a JSON array. Each item: { "criterion": "human-readable description", "action": "click|fill|check|navigate", "selector": "CSS selector", "value": "text to type if action=fill", "expected": "what should happen" }
Use realistic CSS selectors (button, input, [type=text], #id, .class).
      ...
      try {
        switch (step.action) {
          case 'click':
            ...
            break;
          case 'fill':
            await page.fill(step.selector, step.value || 'Test value');
            await page.keyboard.press('Enter');
            break;
          case 'navigate':
            await page.goto(step.selector, { waitUntil: 'networkidle', timeout: 10000 });
            break;
          case 'check':
          default:
...
## Summary
Tested ${testResults.length} acceptance criteria against the live deployment at ${url}.
${passCount}/${testResults.length} passed. Overall result: **${overall}**.
